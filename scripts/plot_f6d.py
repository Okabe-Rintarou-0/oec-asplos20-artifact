...
if len(sys.argv) == 3:
    ...
    print("Usage: python3 plot_f6d.py /path/to/src/ /path/to/dst/")
    exit()

# Equatorial
eq_xvalues = []
eq_yvalues = []
with open(src + 'sim-compose/logs/cs-spacex-eq.csv', 'r') as infile:
...
ns_xvalues = []
ns_yvalues = []
with open(src + 'sim-compose/logs/cs-spacex-ns.csv', 'r') as infile:
    lines = infile.readlines()
    lines = lines[1:]
    for line in lines:
        line_split = line.split(',')
...
un_xvalues = []
un_yvalues = []
with open(src + 'sim-compose/logs/cs-spacex-un.csv', 'r') as infile:
    lines = infile.readlines()
    lines = lines[1:]
    for line in lines:
        line_split = line.split(',')
